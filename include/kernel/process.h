...
struct process {
    ...
    proc_entry_t entry; // entry point
    u32 stack_phys; // physical base of stack (for cleanup)
};

// Allocate a process, set up its virtual address space, and map its stack.
struct process *process_create(proc_entry_t entry);

// Switch to the process's page table and jump to its entry point. Never returns.
[[noreturn]] void process_exec(struct process *p);

// Called when a process's entry function returns.
