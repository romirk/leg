...
static dtb_val_type_t guess_type(const char *name, const u8 *data, u32 len) {
    ...
    static const char *const u32_names[] = {
        ...
        "interrupt-parent", "phandle", "linux,phandle",
        "clock-frequency", "timebase-frequency", "virtual-reg",
        "cache-line-size", "cache-sets", "cache-size", nullptr
    };
    for (u32 i = 0; u32_names[i]; ++i)
        if (strcmp(name, u32_names[i]) == 0 && len == 4)
            return DTB_VAL_U32;

    if (strcmp(name, "clock-output-names") == 0 && len == 8)
        return DTB_VAL_U64;
    ...
    if (all_str && data[len - 1] == 0) {
        u32 nulls = 0;
        for (u32 i = 0; i < len; ++i) if (data[i] == 0) ++nulls;
...
// ---------------------------------------------------------------------------
dtb_err_t dtb_parse(const void *blob, u32 blob_len,
                    dtb_tree_t *tree, dtb_alloc_fn alloc) {
...
// ---------------------------------------------------------------------------
static dtb_node_t *find_in_children(dtb_node_t *node,
                                    const char *segment, u32 seg_len) {
    for (dtb_node_t *c = node->children; c; c = c->next_sibling) {
        u32 name_len = strlen(c->name);
        if (name_len == seg_len && strncmp(c->name, segment, seg_len) == 0)
            return c;
        // Match "foo" against "foo@addr"
        for (u32 i = 0; i < name_len; ++i) {
            if (c->name[i] == '@') {
                if (i == seg_len && strncmp(c->name, segment, seg_len) == 0)
                    return c;
                break;
            }
        }
...
// dtb_reg_entry
// ---------------------------------------------------------------------------
bool dtb_reg_entry(const dtb_node_t *node, u32 index,
                   u64 *base, u64 *size) {
