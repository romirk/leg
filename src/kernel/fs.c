...
const fs_blob_t *fs_blob_at(u32 i) {
    if (i >= g_blob_count) return nullptr;
    return &g_blobs[i];
}

[[gnu::pure]]
const char *fs_blob_name(const fs_blob_t *blob) {
    return g_names + blob->name_offset;
}
...
const fs_blob_t *fs_find(const char *name) {
    ...
    for (u32 i = 0; i < g_blob_count; i++) {
        ...
        if (b->name_size == name_len && strncmp(g_names + b->name_offset, name, name_len) == 0)
            return b;
    }
    return nullptr;
}
